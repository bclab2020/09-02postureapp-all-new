...
biomechanics.renderMusculoskeletalAvatar = function (canvas, c2Cm, th3Cm, s2Cm, options) {
    ...
    ctx.setLineDash([3, 4]);
    ctx.beginPath();
    ctx.moveTo(cx, Math.round(h * 0.04));
    ctx.lineTo(cx, Math.round(h * 0.96));
    ctx.stroke();
    ctx.setLineDash([]);
    ...
    var hipY = Math.round(h * 0.48);
    var s2Y = Math.round(h * 0.45);
    var l3Y = Math.round(h * 0.35);
    var th3Y = Math.round(h * 0.22);
    var c2Y = Math.round(h * 0.13);
    ...
    // 筋肉テンションラインを描画するヘルパー
    function drawMuscle(p1, p2, isOveractive, isInhibited) {
        ctx.beginPath();
    ...
    ctx.quadraticCurveTo(toe.x - 6, toe.y - 5, toe.x - 2, toe.y - 10);
    ctx.lineTo(ankle.x - 4, ankle.y);
    ctx.lineTo(ankle.x + 8, ankle.y);
    ...
    // 鼻と横顔のシルエット
    ctx.beginPath();
    ctx.moveTo(headCenter.x - 10, headCenter.y - 7);
    ctx.lineTo(headCenter.x - 20, headCenter.y);
    ...
    ctx.lineTo(headCenter.x - 30, headCenter.y);
    ctx.stroke();
